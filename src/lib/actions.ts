...
export async function generateRecurringExpenseInstances(
  ...
  if (!expense.isRecurring || !expense.recurringType || !expense.recurringStartDate) {
    // For non-recurring expenses, get active members at the expense date
    const activeMembers = await getActiveMembersForDate(expense.groupId, expense.date)
    const effectiveMembers = expense.splitAll ? activeMembers : expense.expenseMembers.map(em => em.member)
  ...
  while (currentInstanceDate <= currentDate) {
    if (currentInstanceDate >= startDate) {
      ...
      const activeMembers = await getActiveMembersForDate(expense.groupId, currentInstanceDate)
      const effectiveMembers = expense.splitAll ? activeMembers : expense.expenseMembers.map(em => em.member)
